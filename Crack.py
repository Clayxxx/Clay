...
class login:
	...
	def takon(self):
		...
		if takon in ['y','Y','iya']:
			...
			os.system('xdg-open wa.me/6282175721016');exit()
		else:
			login().__login__()
	def bot(self):
...
class menu:
	...
	def main(self):
		...
		jalan(' %s[ %sWalcome om %s%s ]'%(N,H,r,N))
		...
		print(' %s[%sA%s] Get data² facebook'%(N,O,N))
		print(' %s[%sB%s] Lapor bug script'%(N,O,N))
		print(' %s[%sC%s] Keluar, hapus token'%(N,O,N))
		self.pilih()
	...
	def pilih(self):
		...
		if usna in ['']:
			...
			try:
				...
				print(' %s[%s★%s] tidak publik/Ganti token!'%(N,M,N));time.sleep(2);menu().main()
			else:
				crack().fbeh(id)
		elif usna in ['1','01']:
			try:
				...
				os.system('rm -rf token.x')
				exit(' %s[%s★%s] jalankan ulang !'%(N,M,N))
			try:
				print(' %s'%(N))
			...
			try:
				...
				print(' %s[%s★%s] ID %s tidak publik/token rusak'%(N,O,N,idt));time.sleep(3);menu().main()
			else:
				crack().fbeh(id)
		elif usna in ['4','04']:
			...
			try:
				...
				print(' %s[%s!%s] ID %s tidak publik/token rusak'%(N,O,N,idt));time.sleep(2);menu().main()
			else:
				crack().fbeh(id)
		elif usna in ['6','06']:
			print(' %s'%(N))
			...
			exit('%s [%s★%s] Crack telah selesai'%(N,O,N))
		elif usna in ['G','g']:
			target()
		elif usna in ['K','k']:
			nom_wa ='+6282175721016'
			...
			url_wa = ("https://api.whatsapp.com/+6282175721016?phone="+nom_wa+"&text="+kata)
			subprocess.check_output(["am", "start", url_wa])
			exit()
		else:
...
class crack:
	...
	def fbeh(self,id):
		...
		if kham in ['']:
				...
				if pw in ['']:
					...
					def manual(xnxx=None):
						...
						if mani in ['']:
							...
							exit()
					print(' %s '%(N))
					print(' %s [%s1%s] V1Metode Free'%(N,O,N))
					print(' %s [%s2%s] V2Metode Mbasic'%(N,O,N))
					print(' %s [%s3%s] V3Metode Mobile/M'%(N,O,N))
					manual(pw.split(','))
					break
			...
			print(' %s [%s1%s] V1Metode Free'%(N,O,N))
			print(' %s [%s2%s] V2Metode Mbasic'%(N,O,N))
			print(' %s [%s3%s] V3Metode Mobile/Rekomend'%(N,O,N))
			self.otomatis()
	def otomatis(self):
		print(' %s '%(N))
	...
	def free(self):
		print(' %s '%(N))
		print(' %s [%s+%s] akun OK akan di simpan di file  : hasil/okeh.txt'%(N,O,N))
		print(' %s [%s+%s] akun CP akan di simpan di file : hasil/cepeh.txt\n'%(N,O,N))
		with khamdihiXD(max_workers=30) as dihi:
			for me in self.id:
				try:
					uid, name = me.split('<=>')
